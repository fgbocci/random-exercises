...
def show_plane_seats(matrix):
    ...
    for row_number, row in enumerate(matrix):
        print("Row number: " + str(row_number + 1) + " --- " + str(row))
...
def ticket_purchase():
    ...
    while True:
        ...
        if selected_choice == 3:
            print("You've chosen option 3. Exiting the program.")
            break
            ...
            if customer_wants_to_pick_seat:
                ...
                while True:
                    ...
                    print("Random row number: " + str(random_row_number))
                    print("Random col number: " + str(random_column_number))
            ...
            dump_passengers_to_file(passengers)
            # `len(una_lista)  -> cantidad de elementos`
            # str(una_variable) -> su valor como texto
            total_passengers = len(passengers)
            total_plane_seats = 28 * 6
            percentage_occupied = float(total_passengers) / float(total_plane_seats) * 100
            print("Total passengers: " + str(total_passengers))
            print("Total plane seats: " + str(total_plane_seats))
            print("Plane occupied: %.2f" % percentage_occupied)

            print("Option 2 ends the program")
            break


def dump_passengers_to_file(passengers):
    # TODO: format this
    with open("vuelo.txt", "w+") as f:
